...
func (g *Game) moveForward(s float64) {
	if g.getTexNum(int(g.pos.X+g.dir.X*s), int(g.pos.Y)) == 0 {
		g.pos.X += g.dir.X * s
	}

	if g.getTexNum(int(g.pos.X), int(g.pos.Y+g.dir.Y*s)) == 0 {
		g.pos.Y += g.dir.Y * s
	}
}

func (g *Game) moveLeft(s float64) {
	if g.getTexNum(int(g.pos.X-g.plane.X*s), int(g.pos.Y)) == 0 {
		g.pos.X -= g.plane.X * s
	}

	if g.getTexNum(int(g.pos.X), int(g.pos.Y-g.plane.Y*s)) == 0 {
		g.pos.Y -= g.plane.Y * s
	}
}
